...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── 📘 Facebook submenu (A1 / A2 / A3 / S3) ──
    if data == "app_fb":
        kb = await fb_panel_select_inline()
        await safe_edit(query, FB_PANEL_SELECT_TEXT, reply_markup=kb)
        return
    ...
    if data == "select_panel_A1_fb":
        ...
        if not ranges:
            await safe_edit(query,
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("◀️ Back", callback_data="app_fb",
        ...
        if row:
            buttons.append(row)
        buttons.append([InlineKeyboardButton("◀️ Back", callback_data="app_fb",
                                              api_kwargs={"style": "primary"})])
    ...
    if data.startswith("s3change:"):
        ...
        want     = len(current) if current else 2   # replace as many as were assigned (usually 2)
        ...
        new_nums = nums[:want]
        pool[pool_key] = [n for n in nums if n not in new_nums]

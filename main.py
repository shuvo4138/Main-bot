...
def register_handlers(app: Application) -> None:
    ...
    from handlers.balance import balance_command, balance_callback
    ...
    # ── Callbacks ──
    # Balance/withdraw callbacks are registered first with a pattern filter
    # so they're handled here without touching the main callback_handler.
    app.add_handler(CallbackQueryHandler(balance_callback, pattern=r"^wd_"))
...
def main() -> None:
    ...
    # Default HTTPXRequest pool size is 1 — with many concurrent OTP poll
    # loops (A1/S3/S4) all calling send/edit at once, that single connection
    # becomes a bottleneck and causes cascading delays / ReadTimeouts.
    ...
    # get_updates uses long-polling and needs its own dedicated connection
    # so it doesn't compete with regular send/edit calls for a pool slot.

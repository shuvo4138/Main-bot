...
async def job_a3_range_post(context) -> None:
    """
    Fetch live A3 (YesSMS) ranges and post to channel every 2 minutes.
    ...
    from utils.helpers import extract_otp, escape_mdv2
    ...
    try:
        ...
        for r in ranges:
            ...
            otp     = r.get("otp") or extract_otp(message) or ""
            service = r.get("service", "Facebook")

            if not rid or not otp:
                continue

            slot      = now_bd.strftime('%Y-%m-%d %H:%M')
        ...
        logger.error(f"job_a3_range_post error: {e}")
...
def register_jobs(app) -> None:
    jq = app.job_queue
    jq.run_repeating(job_poll_s3_and_shark, interval=60,  first=15,  name="poll_s3_shark")
    jq.run_repeating(job_a1_range_post,     interval=300, first=60,  name="a1_range_post")
    jq.run_repeating(job_a2_range_post,     interval=300, first=90,  name="a2_range_post")

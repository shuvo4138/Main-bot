...
async def a3_get_active_ranges(force: bool = False) -> list[dict]:
    """
    Fetch live ranges from /api/live_access_data.
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            res = await client.get(
                f"{YESMS_BASE_URL}/api/live_access_data",
                headers=_headers(),
            )
        if res.status_code != 200:
            logger.warning(f"A3 live_access_data HTTP {res.status_code}")
            return _ranges_cache["data"]
        ...
        for row in table:
            # row = [range_id, country_emoji_name, message, service, timestamp]
            ...
            # Clean "State of", "Islamic Republic of" etc
        ...
        logger.error(f"a3_get_active_ranges error: {e}")
        return _ranges_cache["data"]
...
async def a3_allocate_number(range_id: str) -> dict | None:
    ...
    try:
        ...
        logger.error(f"a3_allocate_number error: {e}")
        return None
...
async def do_get_number_a3(message, user_id: int, bot=None) -> None:
    ...
    from handlers.helpers import cancel_all_otp_tasks
                ...
                for num in nums:
                    ...
                    try:
                        ...
                        try:
                            ...
                            asyncio.create_task(db_award_otp_balance(user_id))
                        except Exception:
                            pass
                    except Exception as e:

...
async def poll_otps_s3(context) -> None:
    ...
    try:
        ...
        for otp_data in cr_otps:
            try:
                ...
                try:
                    ...
                    logger.error(f"S3 channel send error: {e}")
                    sent_count += 1  # count anyway, don't block inbox
                    ...
                    try:
                        ...
                        if inbox_sent:
                            try:
                                ...
                                asyncio.create_task(db_award_otp_balance(int(uid)))
                            except Exception:
                                pass
                        else:
        ...
        logger.error(f"poll_otps_s3 FATAL error: {e}", exc_info=True)

...
async def fetch_cr_api_otps() -> list[dict]:
    ...
    try:
        ...
        if res.status_code != 200:
            ...
            return []
        data = res.json()
        logger.info(f"CR API raw response type={type(data)} | keys={list(data.keys()) if isinstance(data, dict) else 'list'} | len={len(data) if isinstance(data, list) else 'N/A'} | preview={str(data)[:300]}")
        if isinstance(data, list):
            return data
        # Try common wrapper keys
        for key in ("data", "records", "results", "otps", "sms", "list", "items"):

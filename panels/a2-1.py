...
async def do_get_number_a2(message, user_id: int, bot=None) -> None:
    ...
    # ── Guard: ignore duplicate/overlapping calls (double-tap, slow network
    # retap) — without this, two concurrent fetches can both edit the same
    # card and the numbers shown "randomly" flip to whichever finishes last.
    if user_data[user_id].get("a2_fetching"):
        return
    user_data[user_id]["a2_fetching"] = True

    range_val = user_data[user_id].get("range", "")
    app_name  = user_data[user_id].get("app", "FACEBOOK")

    try:
        await _do_get_number_a2_inner(
            _bot, chat_id, user_id, existing_msg_id, range_val, app_name,
        )
    finally:
        user_data[user_id]["a2_fetching"] = False


async def _do_get_number_a2_inner(
    _bot, chat_id: int, user_id: int, existing_msg_id, range_val: str, app_name: str,
) -> None:
    ...
    if country_code in COUNTRY_NAMES_CODE:
        country_name = COUNTRY_NAMES_CODE[country_code]
        flag         = COUNTRY_FLAGS_CODE.get(country_code, "🌍")
    elif country_code.upper() in COUNTRY_FLAGS_ISO:
        # Some A2/VOLTX ranges are keyed by ISO-2 (e.g. "MN") instead of
        # dial-code — fall back to the ISO tables so the name shows too.
        iso_code     = country_code.upper()
        flag         = COUNTRY_FLAGS_ISO[iso_code]
        country_name = COUNTRY_NAMES_ISO.get(iso_code, user_data[user_id].get("country", "Unknown"))
    else:
        country_name = user_data[user_id].get("country", "Unknown")
        flag         = "🌍"
    ...
    try:
        ...
        for r in ranges:
            ...
            if code in COUNTRY_NAMES_CODE:
                ...
                country_name = COUNTRY_NAMES_CODE[code]
            elif code.upper() in COUNTRY_FLAGS_ISO:
                iso_code     = code.upper()
                range_flag   = COUNTRY_FLAGS_ISO[iso_code]
                country_name = COUNTRY_NAMES_ISO.get(iso_code, code)
            else:
                range_flag   = "🌍"
                country_name = code

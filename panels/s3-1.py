...
# ── Guards concurrent execution so two overlapping poll runs never process the same batch ──
...
def parse_pool_key(pool_key: str) -> tuple[str, str, str | None]:
    """
    Parse a pool_key like "224_fb" or "224_v1_fb".
    Returns (dial_code, service_short, variant|None).
    """
    parts = pool_key.split("_")
    code  = parts[0]
    if len(parts) >= 3 and parts[1] == "v1":
        return code, parts[2].lower(), "v1"
    if len(parts) >= 2:
        return code, parts[-1].lower(), None
    return code, "fb", None
...
def get_button_label(pool_key: str) -> str:
    ...
    v1    = " v1" if variant == "v1" else ""
    return f"{flag} {name} {label}{v1}"
...
def is_shark_pool(pool_key: str) -> bool:
    """Pool keys with _v1 suffix use the Shark (S4) OTP source."""
...
async def remove_number_from_pool(bot, pool_key: str, number: str) -> None:
    ...
    try:
        ...
        logger.info(f"🗑 Deleted: {number} | pool={pool_key}")
    except Exception as e:
...
def s3_find_users_by_number(number: str) -> list[str]:
    ...
    for uid, session in s3_user_sessions.items():
        ...
        if number not in session_numbers:
            continue
        try:
...
async def fetch_cr_api_otps() -> list[dict]:
    """
    Fetch recent OTPs from the CR API endpoint.
    Returns list of {num, message, dt, ...}.
    """
    ...
    try:
        ...
        if res.status_code != 200:
            logger.warning(f"CR API HTTP {res.status_code}")
            return []
        data = res.json()
...
async def poll_otps_s3(context) -> None:
    ...
    if _s3_poll_lock.locked():
        logger.info("S3 poll: previous run still in progress, skipping this tick")
        return
    ...
    async with _s3_poll_lock:
        try:
            ...
            logger.info(f"S3 poll: fetched {len(cr_otps)} OTPs from CR API")
            ...
            for otp_data in cr_otps:
                try:
                    ...
                    # Skip OTPs that predate bot startup
                    try:
                    ...
                    if not otp_code:
                        logger.info(f"S3 — no OTP extracted for {hide_number(number)}, skipping channel post only")
                    else:
                    ...
                    # ── User inbox notification (runs even without an extracted OTP code) ──

...
SERVICE_SELECT_TEXT = "📱 Please select a service:"
...
async def panel_select_inline() -> InlineKeyboardMarkup:
    """
    Main service selection menu — matches screenshot exactly:

      [🆕 Facebook A1]        ← green
      [⚡ Facebook A2]        ← green
      [🔴 Facebook S3 (N)]   ← red
      [📸 Instagram S3 (N)]  ← red/dark
      [💬 WhatsApp]           ← purple
      [✈️ Telegram]           ← purple
    """
...
def after_number_inline_a1(
    ...
    cb_new = f"new_number_{range_val}" if range_val else "back_app"
    buttons.append([InlineKeyboardButton(
        "🔄 Change Numbers", callback_data=cb_new,
        api_kwargs={"style": "success"},
    )])
    buttons.append([InlineKeyboardButton(

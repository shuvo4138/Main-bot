...
async def db_create_withdraw_request(user_id: int, amount: float, bkash: str = "") -> int | None:
    """Insert a pending withdrawal request and deduct balance. Returns its id, or None on failure."""
    try:
        async with httpx.AsyncClient(timeout=10) as client:
...
async def db_set_withdrawal_status(withdrawal_id: int, status: str, admin_id: int) -> dict | None:
    ...
    - approve: balance already deducted at request time — no extra action
    - reject: refund balance to user
    Returns the updated row (has user_id, amount).
    """
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            # Get withdraw details first
            res_get = await client.get(
                _url("withdrawals"),
                params={"id": f"eq.{withdrawal_id}", "select": "user_id,amount,status"},
                headers=_sb_headers(),
            )
            rows_get = res_get.json()
            if not rows_get:
                logger.error(f"db_set_withdrawal_status: withdrawal {withdrawal_id} not found")
                return None
            ...
            if current_status != "pending":
                logger.warning(f"Withdrawal {withdrawal_id} already {current_status}")
                return None
